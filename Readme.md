...
## How to use `esy-rapidjson`?

`esy-rapidjson` can be used from both NPM and directly from Github.

### From NPM 

`esy-rapidjson` is deployed on NPM can be found
[here](https://www.npmjs.com/package/TODO).

You can simply run `esy add esy-rapidjson` to install it, or specify it in
...
{
  "dependencies": {
    "esy-rapidjson": "esy-packages/esy-rapidjson"
  }
}
```
...
```diff
  "dependencies": {
+   "esy-rapidjson": "esy-packages/esy-rapidjson#<commit hash>"
  }
```
